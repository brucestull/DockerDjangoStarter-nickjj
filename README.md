...
```sh
git clone https://github.com/nickjj/docker-django-example docker_django
cd docker_django
...
The app is named `docker_django` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "docker_django" and update a few Docker related resources.
